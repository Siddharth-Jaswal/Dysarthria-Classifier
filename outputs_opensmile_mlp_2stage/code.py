...
class Config:
    ...
    features_cache_path: str = os.path.join("Database", "opensmile_features_final.pkl")
    output_dir: str = "outputs_opensmile_mlp_2stage"
    log_file_name: str = "opensmile_mlp_2stage.log"
    results_csv_name: str = "results_opensmile_mlp_2stage.csv"
...
def setup_logger(cfg: Config) -> logging.Logger:
    logger = logging.getLogger("opensmile_mlp_2stage")
    ...
    if missing_paths:
        ...
        for rel_path, row in tqdm(missing_paths, desc="Extracting"):
            ...
            try:
                ...
                feats = feats_df.to_numpy().squeeze(0)
...
def main():
    ...
    logger.info("Starting openSMILE Hierarchical 2-Stage MLP LOSO Pipeline...")
    ...
    for fold_idx, test_speaker in enumerate(unique_speakers, 1):
        ...
        # --- Targets for Two Stages ---
        y_train_s1 = np.where(y_train == 0, 0, 1)
        
        s2_train_mask = (y_train == 1) | (y_train == 2)
        X_train_s2 = X_train[s2_train_mask]
        y_train_s2 = np.where(y_train[s2_train_mask] == 1, 0, 1)
        
        train_ds_s1 = FeaturesDataset(X_train, y_train_s1)
        train_ds_s2 = FeaturesDataset(X_train_s2, y_train_s2)
        test_ds = FeaturesDataset(X_test, y_test)
        
        train_loader_s1 = DataLoader(train_ds_s1, batch_size=CFG.batch_size, shuffle=True, num_workers=CFG.num_workers)
        train_loader_s2 = DataLoader(train_ds_s2, batch_size=CFG.batch_size, shuffle=True, num_workers=CFG.num_workers)
        test_loader = DataLoader(test_ds, batch_size=CFG.batch_size, shuffle=False, num_workers=CFG.num_workers)
        
        # --- Stage 1 Training ---
        model_s1 = SmileMLP(input_dim=input_dim, num_classes=2).to(CFG.device)
        weights_s1 = compute_class_weights(y_train_s1, 2, CFG.device)
        crit_s1 = nn.CrossEntropyLoss(weight=weights_s1, label_smoothing=0.05)
        opt_s1 = torch.optim.AdamW(model_s1.parameters(), lr=CFG.learning_rate, weight_decay=CFG.weight_decay)
        ...
        for epoch in range(1, CFG.epochs + 1):
            loss_s1, acc_s1, f1_s1 = train_one_epoch(model_s1, train_loader_s1, crit_s1, opt_s1, CFG.device)
            if f1_s1 > best_f1_s1:
                best_f1_s1 = f1_s1
                pat_s1 = 0
                best_state_s1 = {k: v.cpu() for k, v in model_s1.state_dict().items()}
            else:
                pat_s1 += 1
                if pat_s1 >= CFG.early_stopping_patience:
                    break
        model_s1.load_state_dict(best_state_s1)
        
        # --- Stage 2 Training ---
        model_s2 = SmileMLP(input_dim=input_dim, num_classes=2).to(CFG.device)
        weights_s2 = compute_class_weights(y_train_s2, 2, CFG.device)
        crit_s2 = nn.CrossEntropyLoss(weight=weights_s2, label_smoothing=0.05)
        opt_s2 = torch.optim.AdamW(model_s2.parameters(), lr=CFG.learning_rate, weight_decay=CFG.weight_decay)
        ...
        for epoch in range(1, CFG.epochs + 1):
            loss_s2, acc_s2, f1_s2 = train_one_epoch(model_s2, train_loader_s2, crit_s2, opt_s2, CFG.device)
            if f1_s2 > best_f1_s2:
                best_f1_s2 = f1_s2
                pat_s2 = 0
                best_state_s2 = {k: v.cpu() for k, v in model_s2.state_dict().items()}
            else:
                pat_s2 += 1
                if pat_s2 >= CFG.early_stopping_patience:
                    break
        model_s2.load_state_dict(best_state_s2)
        
        logger.info(f"Stage1 Train Metrics -> Best F1: {best_f1_s1:.4f}")
        logger.info(f"Stage2 Train Metrics -> Best F1: {best_f1_s2:.4f}")
        print(f"Stage1 Train Best F1: {best_f1_s1:.4f}")
        print(f"Stage2 Train Best F1: {best_f1_s2:.4f}")
        ...
        with torch.no_grad():
            for X_b, _ in test_loader:
                ...
                out_s1 = torch.argmax(model_s1(X_b), dim=1)
                out_s2 = torch.argmax(model_s2(X_b), dim=1)
                
                for s1, s2 in zip(out_s1, out_s2):
                    if s1.item() == 0:
                        fold_preds.append(0)  # Healthy
                    else:
                        if s2.item() == 0:
                            fold_preds.append(1)  # Mild
                        else:
                            fold_preds.append(2)  # Severe
                            
        fold_preds = np.array(fold_preds)
        
        final_acc = accuracy_score(y_test, fold_preds)
        final_bal_acc = balanced_accuracy_score(y_test, fold_preds)
        final_macro_f1 = f1_score(y_test, fold_preds, average="macro", zero_division=0)
        
        pred_counts = np.bincount(fold_preds, minlength=CFG.num_classes)
        pred_dist_str = "\n".join([f"    {CFG.class_names[i]}: {count}" for i, count in enumerate(pred_counts)])
        
        epoch_msg = (
            f"\nFinal Test Metrics (Speaker {test_speaker}):\n"
            f"  Accuracy: {final_acc:.4f}\n"
            f"  Balanced Acc: {final_bal_acc:.4f}\n"
        ...
        all_test_speakers.extend([test_speaker] * len(y_test))
