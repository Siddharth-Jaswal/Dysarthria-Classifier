...
class Config:
    ...
    output_dir: str = "outputs_opensmile_mlp_final"
    log_file_name: str = "opensmile_mlp_final.log"
    results_csv_name: str = "results_opensmile_mlp_final.csv"
...
def setup_logger(cfg: Config) -> logging.Logger:
    logger = logging.getLogger("opensmile_mlp_train")
...
def main():
    ...
    logger.info("Starting openSMILE + MLP LOSO Pipeline...")
    ...
    for fold_idx, test_speaker in enumerate(unique_speakers, 1):
        ...
        model = SmileMLP(input_dim=input_dim, num_classes=CFG.num_classes).to(CFG.device)
